...
// ─────────────────────── RSVP CONFIG ──────────────────
const RSVP_ENDPOINT = 'https://script.google.com/macros/s/AKfycbxHstEYTz9mJL5KBX76eX_N4oibGz1XdCvjgSmiP-2xfrolFOAeCdIm-h2KX1RKWHBCvw/exec';
...
(function initRSVP() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    const attendanceChecked = form.querySelector('input[name="attendance"]:checked');
    ...
    fetch(RSVP_ENDPOINT, {
      ...
      .catch(() => { /* opaque response — treat as success */ })
      .finally(() => {
        // ── Gold dust burst ──
        ...
        // ── Clear form fields ──
        form.reset();
        document.querySelectorAll('.radio-option').forEach(o => (o.style.color = ''));
  ...
  document.querySelectorAll('.radio-option').forEach(opt => {
    opt.addEventListener('change', () => {
      document.querySelectorAll('.radio-option').forEach(o => (o.style.color = 'var(--navy)'));
...
(function initCoin() {
  ...
  function makeFaceTexture(isFront) {
    const S = 512, cx = S / 2, cy = S / 2, r = S / 2 - 4;
  ...
  const coin = new THREE.Mesh(
    new THREE.CylinderGeometry(2.3, 2.3, 0.2, 80),
    [sideMat, frontMat, backMat]
  );
  spinGroup.add(coin);
  ...
  if (typeof gsap !== 'undefined' && typeof ScrollTrigger !== 'undefined') {
    ...
    tl
      .to(scrollGroup.position, { x: -3.8 * amp, y: -2.2 * amp, z: 0.2, duration: 0.25 }) // details
      .to(scrollGroup.position, { x: 3.6 * amp, y: 2.0 * amp, z: 0.0, duration: 0.25 }) // countdown
      .to(scrollGroup.position, { x: 3.6 * amp, y: -2.2 * amp, z: 0.3, duration: 0.25 }) // dresscode
      .to(scrollGroup.position, { x: -3.8 * amp, y: 2.0 * amp, z: 0.5, duration: 0.25 }); // rsvp

    // Tilt variation (parallel from t=0)
    tl
      .to(scrollGroup.rotation, { x: 0.1, z: -0.08, duration: 0.5 }, 0)
      .to(scrollGroup.rotation, { x: 0.4, z: 0.12, duration: 0.5 }, 0.5);

    // Slight scale variation during scroll for depth feel
    const maxScale = baseScale() * 1.17;
    tl.to(scrollGroup.scale, { x: maxScale, y: maxScale, z: maxScale, duration: 1 }, 0);
